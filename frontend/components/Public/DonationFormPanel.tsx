...
import { fallbackDonationPaymentSettings, type DonationPaymentSettings } from "@/lib/cms/donationSettingsShared";
import { publicDonationFormSchema } from "@/lib/validation/donation";
import { paymentMethodRequiresUtr, registerPublicDonation } from "@/services/api/donations";
import type { Donation, DonationFormValues } from "@/types/models/donation";

export default function DonationFormPanel({
  paymentSettings = fallbackDonationPaymentSettings,
}: {
  paymentSettings?: DonationPaymentSettings;
}) {
  ...
  const submitDonation = async () => {
    ...
    setError(null);
    try {
      const parsed = publicDonationFormSchema.safeParse(values);
      if (!parsed.success) {
        const first = parsed.error.issues[0];
        setError(first?.message ?? "Please check the donation form.");
        return;
      }
      if (paymentMethodRequiresUtr(values.payment_method) && !values.transaction_reference.trim()) {
        setError("UTR / transaction reference is required for this payment method.");
        return;
      }
        ...
        <div className="grid gap-4 sm:grid-cols-[9rem_1fr] sm:items-center">
          {paymentSettings.qrImageUrl ? (
            <img
              ...
              alt={`${paymentSettings.accountName} donation UPI QR code`}
              className="h-36 w-36 rounded-lg border border-gray-200 bg-white object-contain p-2"
            />
          ) : (
            <div className="flex h-36 w-36 items-center justify-center rounded-lg border border-dashed border-gray-300 bg-gray-50 p-3 text-center text-xs leading-5 text-gray-600">
              Upload the UPI QR image in CMS (Donation Payment Setting → qrImage).
            </div>
          )}
          <div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {paymentSettings.paymentOptions
            ...
            .map((option) => (
              <span
                key={option.value}
                className="inline-flex items-center gap-1.5 rounded-full border border-gray-200 bg-white px-3 py-1 text-xs font-bold text-gray-800 shadow-sm"
              >
